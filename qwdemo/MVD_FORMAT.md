...
**Synthesis recovery (analytics layer, not protocol-level)**: The full pickup count can be recovered without `//ktx took` by combining three signals all transported on the wire: (1) a deterministic respawn-time table per item kind, (2) per-slot `svc_updatestat` deltas — the picking player's stats always tick up at touch, regardless of any visibility transition, and (3) `svc_playerinfo` origin samples for proximity. After every observed `Taken=true(ent, T)` at the entity-state layer, schedule a synthesis check at `T + respawnSec[kind]`; if a unique player has stat-delta evidence consistent with the kind near that time *and* their position was within touch radius of the entity origin, record a synthetic pickup at the predicted instant and chain forward. The qwanalytics implementation lives in [`qwanalytics/analyzer/items.go`](../qwanalytics/analyzer/items.go) (`processSyntheticRespawns` / `findSyntheticPicker`); per-corpus this closes most of the under-count gap on hotly-contested 1on1 and 4on4 demos. The chain terminates naturally when the wire eventually shows `Taken=false` (the entity actually became visible again), which in items.go cancels the schedule. MH is excluded from the chain because its predicted respawn depends on holder-rot timing, handled separately.
