...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getOverview",
		Description: "Return a curated summary of the demo (map, teams, top streaks, top powerups). Who won: `teams` and `players` are sorted by frags descending, so teams[0] (team modes) / players[0] (duel/FFA) is the winner — frags is the canonical net score. Also carries `errors`: the analyzer's non-fatal errors — if non-empty the result is degraded (some sections may be missing/partial), so check it before trusting detail views. Use this first to decide which detailed view to query next. UNITS SEAM: overview times (duration, matchStart/End, topStreaks/topPowerups start+duration) are integer MILLISECONDS, while every tool's startTime/endTime/time input is match-relative SECONDS — divide by 1000 when carrying an overview time into a filter. Response shape: see mvd-api /v1/demos/{id}/overview.",
	...
	addTool(s, &mcp.Tool{
		Name:        "getDamage",
		Description: "Per-hit damage aggregates + log, reconstructed from the KTX damage stream. TWO damage families: 'raw' = the unbound wire values (overkill included, so totals run HIGHER than the KTX scoreboard, which caps damage to victim health); 'bounded' = KTX-scoreboard semantics reconstructed per hit (armor absorbed + health capped to the victim's remaining health), so per-player given/taken land near-equal to demoInfo dmg. Pick a family with dmg=raw|bounded|both. Default: summary defaults TRUE at this layer, and an unset dmg then yields BOTH families side by side (the aggregates are small, so it is cheap and answers more) — dmg=both adds a per-player `bounded` nest (the same damage-figure fields under bounded semantics) and a per-event `bounded`; a full-log request (summary:false) defaults instead to dmg=raw (the lean v53 shape). A defaulted summary response carries a hint — pass summary:false for the per-hit events log. dmg=bounded errors (bounded_unavailable) on midair / instagib / dmgfrags demos, whose server mode rewrites the take in ways the wire doesn't expose; boundedMode echoes 'standard' or 'skipped:midair|instagib|dmgfrags', and every bounded field is absent when skipped. The per-event `bounded` is OMITTED when it equals the raw damage (the common no-overkill case; note 0 is a real value — a fully-nullified pent/teamplay hit). Optional players= / weapons= / startTime/endTime filters; any filter recomputes every aggregate from the filtered log. Positional instant kills — telefrags (the 9999 instakill sentinel) and stomps (head-stomps) — are EXCLUDED from the per-weapon damage figures (byWeapon, matrix, EWep, totalDamage) and listed separately (telefrags/stomps lists + per-player counts), BUT their damage DOES fold into given/givenTeam/taken in both families, matching KTX; each telefrags[]/stomps[] entry carries that folded value as `bounded` (and `damage` when the raw fold differs). Shape vocabulary: totalDamage + byWeapon (enemy damage per attacker weapon) + byPlayer (given (to enemies), taken (all sources), givenTeam, givenSelf, takenEnv, byWeapon, and the EWep victim-weapon buckets enemyVsSg/enemyVsMid/enemyVsLg/enemyVsRl/enemyVsBoth where ewep=lg+rl+both = damage to enemies holding RL/LG) + matrix (attacker->victim totals) + scoreboard (stream-vs-KTX cross-check; carries a bounded side for the like-for-like compare). Empty-log convention: events is null when dropped by summary, [] when included but the filter matched nothing. Related: getEvents(types:['damage']) is the raw hit feed; getEvents(types:['telefrag','stomp']) the positional-kill events.",
