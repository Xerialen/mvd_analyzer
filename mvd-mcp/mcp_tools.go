...
func registerTools(s *mcp.Server, b MCPBackend, sr searcher) {
	...
	addTool(s, &mcp.Tool{
		Name:        "getDamage",
		Description: "Per-hit damage aggregates + log, reconstructed from the KTX damage stream. summary defaults TRUE at this layer (aggregates only; the response carries a hint) — pass summary:false for the per-hit events log. Optional players= / weapons= / startTime/endTime filters; any filter recomputes every aggregate from the filtered log. Positional instant kills — telefrags (the 9999 instakill sentinel) and stomps (head-stomps) — are EXCLUDED from all damage figures and listed separately (telefrags/stomps lists + per-player counts). NOTE: damage is UNBOUND (includes overkill), so totals run higher than the KTX scoreboard, which is bounded to victim health; scoreboard carries the stream-vs-KTX cross-check. Shape vocabulary: totalDamage + byWeapon (enemy damage per attacker weapon) + byPlayer (given (to enemies), taken (all sources), givenTeam, givenSelf, takenEnv, byWeapon, and the EWep victim-weapon buckets enemyVsSg/enemyVsMid/enemyVsLg/enemyVsRl/enemyVsBoth where ewep=lg+rl+both = damage to enemies holding RL/LG) + matrix (attacker->victim totals). Empty-log convention: events is null when dropped by summary, [] when included but the filter matched nothing. Related: getEvents(types:['damage']) is the raw hit feed; getEvents(types:['telefrag','stomp']) the positional-kill events.",
