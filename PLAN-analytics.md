...
| F25 | Post-match reconnect clobbered the tracked scoreboard: svc_updatefrags re-inits a returning slot to 0 during intermission and last-write-wins FragsBySlot kept it (hub 212483 Doomie 34→0, 212545 squeeze 55→0; the pre-v48 0-frag filter had masked this by dropping the players — v48's F6 fix surfaced the corrupted zeros). MatchAnalyzer now tracks frags itself, frozen at `timing.Ended`; `Context.FragsBySlot` deleted; mid-match reconnects unaffected (KTX re-asserts the restored count). Found in user QA of the phase-1 golden diff. | P5.5 |
